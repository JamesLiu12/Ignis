...
EditorApp::EditorApp()
{
	...
	Log::CoreInfo("EditorApp initialized");
	...
	console_panel->AddMessage(ConsoleMessageLevel::Info, "Scene ready - use Properties panel to load a model");
	
	Log::CoreInfo("Editor panels registered");
	Log::CoreInfo("Editor is ready");
}
...
EditorApp::~EditorApp()
{
	...
	auto& core_sinks = Log::GetCoreLogger()->sinks();
	auto& client_sinks = Log::GetClientLogger()->sinks();
